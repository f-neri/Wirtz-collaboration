...
plot_single_marker <- function(data, y, ylab, title, test_results, ymax_multiplier = 1.1) {
  ...
  ggplot(data,
    ...
    facet_grid(Serum ~ .) +
    coord_cartesian(ylim = c(NA, test_results$ymax %>% max(.) * (ymax_multiplier + 0.1))) +
...
# Compare IR vs CTL samples for yH2AX
single_marker_Morph_test_results <- df_single_marker_Morph %>%
  # select df with both FS and SS
  filter(Serum == "FS_SS") %>%
  .$data %>%
  .[[1]] %>%
  # calculate adj-pvalues
  calculate_test_results(
  variables = c("Narea_overall", "NSF_overall"),
  grouping = c("Serum", "Cell.Type")
  ) %>%
  # calculate x:y coordinates for stat annotations
  calculate_annotation_position(
...
Fig_PopData_v6.3 <- Panel_Representative_Images +
  ...
  SABGal_EdU_plots$FS_SS_EdU + ICC_2ndMarkers$FS_SS_LaminB1 + ICC_2ndMarkers$FS_SS_HMGB1 + Morph_plots$FS_SS_SF +
  plot_layout(design = layout2,
              guides = "collect") +
  plot_annotation(tag_levels = list(c("a", "b", "c", "", "d", "", "", "", "", ""))) &
  theme(plot.title = element_text(size = 10),
...
save_pdf <- function(figure, name) {
  ...
  pdf(paste0("inst/figures/Population-Analysis_",name,"_", Sys.Date(), ".pdf"),
    width = 7.5,
    height = 10,
    family = "sans",
    compress = F)
