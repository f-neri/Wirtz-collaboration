...
```{r print population data SABGal EdU, eval=FALSE}

# print population level data for SABGal EdU staining
df_single_marker %>%
  # extract df with both Serum conditions
  filter(Serum == "FS_SS") %>%
  pluck("data", 1) %>%
  # remove redundant Serum_Condition col
  select(-Serum_Condition) %>%
  # print as excel
  openxlsx::write.xlsx(x = ., file = file.path("inst","population-data_SABGal-EdU.xlsx"))

```


```{r n count}
df_single_marker$data %>%
  .[[3]] %>%
  group_by(Cell.Type, Condition, Serum) %>%
...
```{r print population data ICC, eval=FALSE}
...
df_ICC_Morph_Serum %>%
  ...
  pluck("data", 1) %>%
  # print as excel
  openxlsx::write.xlsx(x = ., file = file.path("inst","population-data_ICC.xlsx"))
...
```{r n count}
# yH2AX
df_ICC_Morph_Serum$data[[which(df_ICC_Morph_Serum$Serum == "FS_SS")]] %>%
  group_by(Cell.Type, Condition, Serum) %>%
  summarise(well_count = n())

# p21, LaminB1, HMGB1
df_ICC_Morph_Serum_2ndMarker %>%
  filter(Serum == "FS_SS",
         Channel3 == "p21") %>%
  .$data %>%
  .[[1]] %>%
  group_by(Cell.Type, Condition, Serum, Channel3) %>%
  summarise(well_count = n())
...
```{r n well count}
df_single_marker_Morph$data[[which(df_single_marker_Morph$Serum == "FS_SS")]] %>%
  group_by(Cell.Type, Condition, Serum) %>%
